...
const seedInterviewQuestions = async () => {
    try {
        ...
        await InterviewQuestion_1.default.deleteMany({});
        console.log('🗑️  Deleted existing interview questions');
        // Insert sample questions
        await InterviewQuestion_1.default.insertMany(sampleQuestions);
        console.log(`✅ Seeded ${sampleQuestions.length} interview questions successfully!`);
        // Show count by category
        const javaScriptCount = await InterviewQuestion_1.default.countDocuments({ category: 'javascript' });
        const pythonCount = await InterviewQuestion_1.default.countDocuments({ category: 'python' });
        const generalCount = await InterviewQuestion_1.default.countDocuments({ category: 'general' });
